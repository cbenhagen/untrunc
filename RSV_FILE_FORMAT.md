...
2. **Video essence** - H.264/AVC or H.265/HEVC video frames (variable count, typically 12 frames)
3. **Audio essence** - PCM audio samples (size calculated from GOP duration)
...
- **Typical frames per GOP**: 12 (but can vary, especially during scene changes)
- **Frames per GOP**: Auto-detected from the RSV file itself
...
### Variable GOP Sizes
While most GOPs contain 12 frames, some may contain more (observed up to 31 frames). This can occur during:
- Scene changes
- Flash/strobe events
- Rapid motion changes

When recovering RSV files, do not assume exactly 12 frames per GOP.
...
### Chunk Structure
- **Chunk size**: Variable (calculated dynamically from GOP duration)
...
- **Duration per chunk**: Matches GOP duration

### Calculation
Audio chunk size is calculated dynamically based on GOP duration:
```
...
The implementation auto-detects GOP size and calculates audio chunk size accordingly.
...
3. **Auto-detection**:
   ...
   - Audio chunk size is calculated dynamically based on detected GOP duration
...
6. **Handle variable GOPs**: 
   - Don't assume exactly 12 frames per GOP - count actual AUD patterns
   - Don't assume exactly 12 rtmd packets - count consecutive valid packets
   - Avoid false positive rtmd matches by verifying the Sony tag at offset 8
   - Audio chunk size adapts automatically to GOP duration
...
| GOP size | 12 frames (variable, auto-detected) |
...
4. **Audio Chunk Size**: Calculated dynamically from detected GOP duration and audio sample rate
